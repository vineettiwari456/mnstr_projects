...
class ResumeDocstore:
    ...
    def insert_dove_db_old(self, records, foldarg):
        insert_query = """insert into user_active_resume_text_{0} (`resume_text`,`kiwi_profile_id`,`user_id`,`profile_id`,`user_active_data_id`,`status`) values(%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE resume_text=%s;""".format(foldarg)
        conn, curr = self.dove_insert_connection()
        for rec in records:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=30) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_url):
                ...
                try:
                    ...
                    print(exc)
                    pass
        print("Total inserted resume text :",len(Total_records),Total_records)

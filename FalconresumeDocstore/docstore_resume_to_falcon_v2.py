...
class ResumeDocstore:
    ...
    def insert_falcon_db(self, records):
        insert_query = """insert into user_active_resume_text_temp (`resume_text`,`kiwi_profile_id`,`user_id`,`profile_id`,`user_active_data_id`,`status`) values(%s,%s,%s,%s,%s,%s);"""
    ...
    def getLoginDetails(self):
        sum = 0
        maindata = []
        for arg_val in range(8, 10):
            query_count = "select count(*) as count from user_active_data as uad inner join user_profiles as up on uad.profile_id=up.id where (mod(uad.id,10) = %s) and up.resume_exists=1 and uad.created_at between 1580495400000 and 1583389800000;" % (
                str(arg_val))

...
class ResumeDocstore:
    ...
    def download_doc(self, url_link, kiwi_profileid, fprofile_id, fuser_id, fuseractivedataid, status, isupdate):
        temp = []
        resume_text = ""
        try:
            cc = 0
            ...
            print(e, url_link)
            logfilename = main_directory + "/insert_error_log"
            ...
            pass
        if len(resume_text) > 5 and not isupdate:
    ...
    def update_dolphin_db(self, update_records, table_index):
        ssourceval = [(table_index, updat_data[0], updat_data[5], updat_data[3]) for updat_data in update_records]
        try:
            conn, curr = self.dove_insert_connection()
            update_query = "update user_active_resume_text_%s set resume_text=%s,status=%s,updated=now() where profile_id=%s;"
            curr.executemany(update_query, ssourceval)
            conn.commit()
            conn.close()
        except Exception as exc:
            self.insert_log_into_table(updat_data[3], "Update DB Error: " + str(exc))
            pass
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=30) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_url):
                ...
                try:
                    ...
                    print(exc)
                    pass
        print("Total count :", len(Total_records))
    ...
    def check_exist(self, alluaddata, tableindex):
        ...
        query = "select id,profile_id from user_active_resume_text_{0} where profile_id in ({1}) ;".format(tableindex,
                                                                                                           profilesids)
        # print(query)
        curr.execute(query)
        ...
        for arg_val in range(0, 10):
            ...
            while lastactive_at_time and is_next:
                ...
                try:
                    ...
                    query = "select uad.id as useractivedataid,uad.user_id,uad.profile_id,up.kiwi_profile_id,up.resume_exists as enabled,uad.active_at as activedate from user_active_data as uad inner join user_profiles as up on uad.profile_id=up.id where (mod(uad.profile_id,10) = {0}) and uad.active_at>{1} order by uad.active_at asc LIMIT {2};".format(
                    ...
                    if len(useractivedata) > 0:
                        process_data, need_to_update_data = self.check_exist(useractivedata, arg_val)
                        print("Process for insert data length ::",
                              str(len(process_data)) + " Date : " + str(datetime.now()) + " Table index : " + str(
                                  arg_val))
                        ...
                        if len(need_to_update_data) > 0:
                            self.kiwi_profile_ids(need_to_update_data, arg_val, is_update=True)
                    # is_next = False
